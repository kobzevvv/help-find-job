...
export class LoggingService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      await this.db.exec(
        `CREATE TABLE IF NOT EXISTS user_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT NOT NULL, level TEXT NOT NULL, event_type TEXT NOT NULL, user_id INTEGER, chat_id INTEGER, message TEXT NOT NULL, data TEXT, error_details TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`
      );
  ...
  async logAIAnalysis(
    ...
    await this.log(
      'INFO',
      'AI_ANALYSIS',
      `AI analysis ${success ? 'succeeded' : 'failed'} in ${duration}ms`,
  ...
  async logWebhookReceived(
    ...
    await this.log(
      'DEBUG',
      'WEBHOOK_RECEIVED',
      `Received ${updateType} update ${updateId}`,
      { updateId, updateType, ...data },
      userId,
      chatId
    );
  }
  ...
  async getFormattedRecentLogs(
    ...
    if (formattedMessage.length > 4000) {
      formattedMessage =
        formattedMessage.substring(0, 3900) +
  ...
  private calculateTimeRange(logs: LogEntry[]): string {
    ...
    try {
      ...
      if (!newestLog?.timestamp || !oldestLog?.timestamp)
  ...
  async getAdminLogSummary(hours: number = 24): Promise<string> {
    ...
    try {
      ...
      const stats = result.results as Array<{
        level: string;
        count: number;
        event_type: string;
      }>;
      ...
      const warnCount = stats
        .filter((s) => s.level === 'WARN')
        .reduce((sum, s) => sum + s.count, 0);
      const infoCount = stats
