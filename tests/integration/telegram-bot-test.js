...
describe('Telegram Bot Integration Tests', () => {
    ...
    beforeAll(() => {
        ...
        mockSessionService = new SessionService(mockKV, 3600);
        ...
        loggingService = new LoggingService(mockDB);
        ...
        jest.spyOn(mockTelegramService, 'sendMessage').mockImplementation(async (params) => {
            console.log('📱 Mock Telegram message:', params.text?.substring(0, 100) + '...');
            return true;
        });
        conversationHandler = new ConversationHandler(mockSessionService, mockTelegramService, documentService, aiService, enhancedAIService, loggingService, 'test', 'testpassword');
    });
    beforeEach(() => {
        jest.clearAllMocks();
    });
    describe('Resume-Job Matching Commands', () => {
        ...
        test('should handle /resume_and_job_post_match command', async () => {
            ...
            expect(mockTelegramService.sendMessage).toHaveBeenCalledWith(expect.objectContaining({
                chat_id: testChatId,
...
//# sourceMappingURL=telegram-bot-test.js.map